...
<ul>
  <li>
    <a href="./nsxt-install-prereqs.html">NSX Installation Prerequisites</a>
  </li>
  <li>
    <a href="./nsxt-install-managers.html">Install and Configure the NSX Manager Hosts</a>
  </li>
  <li>
    <a href="./nsxt-install-tls-certs.html">Generate and Register the NSX TLS Certificate and Private Key</a>
  </li>
  <li>
...
##<a id='overview'></a> Overview of Transport Zones for NSX
...
match exactly the **Edge Switch Name** value that you specify when you configure an NSX Edge Node for use with {{ vars.product_short }}.
...
<table>
  ...
  <tr>
    <td>Option 3: Use the NSX API</td>
    <td>Yes customization</td>
    <td>Yes customization</td>
  </tr>
</table>
...
<p class="note"><strong>Note:</strong> In NSX 3.1 and later, the Transport Zone Host Switch Name has been deprecated and removed from the NSX configuration UI.
  For more information, see <a href="#nsxt31-edge-config">{{ vars.product_short }} NSX Edge Switch and Transport Zone Host Switch Name Requirements</a>.</p>

##<a id='option1'></a> Configure Your NSX Transport Zones for {{ vars.product_short }}

{{ vars.product_short }} requires the NSX **Edge Switch Name** and the Transport Zone host switch name to be identical.
...
###<a id='option2'></a> Option 2: Create Custom Transport Zones and Use the NSX API to Get the Host Switch Names

If you want to create a custom Transport Zone, you can do so using the NSX user interface.
In this case, because the host switch name is deprecated from the NSX user interface, you have to make an NSX API call to get the host switch name so that you can configure the Edge Nodes with the correct switch.

When you create a custom Transport Zone using the NSX web interface,
NSX will generate the associated switch name for you.

To create custom Transport Zones using the Host Switch names:

1. Create a custom Overlay Transport Zone:
    1. In the NSX Management Console, navigate to **System** > **Fabric** > **Transport Zone**.
    1. Click **Add**.
...
<br>
        ...
        $ curl -k -u user:password -X GET "https://10.20.30.40/api/v1/transport-zones"
        {
          ...
          "sort_ascending" : true
        ```
1. Configure NSX for the Edge Nodes:
...
###<a id='option3'></a> Option 3: Use the NSX API to Create Custom Transport Zones and NSX Switches
...
<br>
    ...
    ```
1. Create a custom VLAN Transport Zone and associated custom NSX switch:
...
<br>
    ...
    ```
1. Use the retrieved values to configure NSX as described in the **Configure NSX for the Edge Node** step in [Option 2: Create Custom Transport Zones and Use the NSX API to Get the Host Switch Names](#option2) above.

##<a id='nsxt31-edge-config'></a> {{ vars.product_short }} NSX Edge Switch and Transport Zone Host Switch Name Requirements

In NSX 3.1 and later, the Transport Zone Host Switch Name has been deprecated and removed from the NSX configuration UI.

For {{ vars.product_short }}, the NSX **Edge Switch Name** and the Transport Zone host switch name must be identical.
When configuring NSX, configure the **Edge Switch Name** to be the same as the Transport Zone host switch name.

<p class="note"><strong>Note:</strong> The NSX 3.x Edge Node configuration displays the following message
