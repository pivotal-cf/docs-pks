...
by {{  vars.product }} with NSX. For each load balancer service, NCP, by way of the Kubernetes CustomResourceDefinition (CRD),
creates corresponding NSXLoadBalancerMonitor objects.

By default {{  vars.product }} deploys the following NSX virtual servers for each Kubernetes cluster:
...
{{# evalExpression "current_page.data.lbtype == 'layer7'"}}
For information about configuring layer 7 ingress routing load balancers see
...
[Create an IP Pool in Manager Mode](https://techdocs.broadcom.com/us/en/vmware-cis/nsx/nsxt-dc/3-2/administration-guide/manager-mode/advanced-grouping-objects/create-an-ip-pool.html)
or [Add an IP Address Pool](https://techdocs.broadcom.com/us/en/vmware-cis/nsx/vmware-nsx/4-1/administration-guide/ip-address-management-ipam/add-an-ip-address-pool-in-nsx.html)
