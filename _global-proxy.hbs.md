...
<br>
1. (Optional) Configure {{  vars.product }} to use a proxy.
<br>
<br>
...
<br>
Configure {{  vars.product }} to use your proxy and activate the following:
  * TKGI API access to public Internet services and other internal services.
  * {{  vars.product }}-deployed Kubernetes nodes access to public Internet services and other internal services.
  * {{  vars.product }} Telemetry ability to forward Telemetry data to the CEIP and Telemetry program.
...
1. To complete your global proxy configuration for all outgoing HTTP/HTTPS traffic from your Kubernetes clusters, perform the following steps:
    ...
    allow for internal {{  vars.product }} communication.
    <br>
    Include `127.0.0.1` and `localhost` in the **No Proxy** list.
    <br>
    Also include the following in the **No Proxy** list:
        * Your {{  vars.product }} environment's CIDRs, such as
    the service network CIDR where your {{  vars.product }} cluster is deployed,
    the deployment network CIDR, the node network IP block CIDR, and the pod network IP block CIDR.
    <br>
        * The FQDN of any registry, such as the Harbor API FQDN, or component communicating with {{  vars.product }}, using a hostname
        instead of an IP address.
    <br>
    {{# evalExpression "current_page.data.topic=='proxies-nsx-t' || current_page.data.iaas == 'vSphere'"}}
    ...
    <br>
        ...
        *.example1.com,
        ...
        ```
        {{/ evalExpression }}
    {{# evalExpression "current_page.data.topic=='proxies-aws'"}}
    ...
    and your {{  vars.product }} FQDN are not proxied. This allows internal {{  vars.product }} communication.
    <br><br>
