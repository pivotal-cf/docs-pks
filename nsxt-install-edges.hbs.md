---
title: Install and Configure the NSX Edge Nodes
 -NSXT
---

This topic describes how to deploy and configure NSX-T Data Center v3.0 NSX-T Edge Nodes for use with {{  vars.product_full }} on vSphere.
...
##<a id='nsxt30-edge-nodes'></a> Deploy NSX Edge Nodes
...
Deploy the Edge Node 1 VM using the NSX Manager interface.
...
1. Configure **Credentials** as follows:
  - **CLI User Name**: `admin`
  - **CLI Password**: Enter a strong password for the `admin` user that complies with the NSX requirements.
  - **Enable SSH Login**: Yes
  - **System Root Password**: Enter a strong password for the `root` user that complies with the NSX requirements.
...
1. Once the process completes, use the NSX Manager to ensure the Edge Node 1 has deployed successfully.
...
Deploy the Edge Node 2 VM using the NSX Manager interface.
...
1. Once the process completes, use the NSX Manager to ensure the Edge Node 2 has deployed successfully.
...
[Deploy NSX Transport Nodes](./vsphere-nsxt-index.html).
