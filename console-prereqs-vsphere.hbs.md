...
- RAM: 8&nbsp;GB
- Disk: 40&nbsp;GB
- Virtual NIC (vNIC) must be assigned to a network with connectivity to vCenter and NSX Manager if {{  vars.product }} uses NSX Data Center as the container networking interface.

<p class="note"><strong>Note</strong>: The OVA requirements described here are the minimum supported configuration.</p>

The following vSphere clusters must exist in the target vCenter Server data center before you can deploy {{  vars.product }} from the management console:
