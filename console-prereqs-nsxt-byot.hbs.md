...
This topic provides requirements for installing the {{  vars.product }} Management Console (TKGI MC)
in a bring your own topology environment.

##<a id='overview'></a> Overview

A bring your own topology environment is an VMware NSX instance that you have fully configured yourself for use with {{  vars.product }}.
For example, an VMware NSX instance that you have used in a previous deployment of {{  vars.product }}.
...
<p class="note"><strong>Note</strong>: In BYOT mode, {{  vars.product }} Management Console automatically retrieves the tier0 HA mode from your VMware NSX environment and
creates NAT rules on the tier 0 or tier 1 router.</p>

## <a id='general-requirements'></a> General Requirements

The following are requirements for deploying the {{  vars.product }} Management Console:

- Do not use the network on which you deploy the {{  vars.product }} Management Console VM
as the network for the management plane when you deploy {{  vars.product }}.
...
- A router. The TKGI Management Console supports Tier-0 and Tier-1 routers in either Active-Active or Active-Standby mode.
    <p class="note"><strong>Note</strong>: If you are deploying {{  vars.product }} in a multiple-tier0 topology,
    	additional post-deployment configuration of the management console VM is required. For information,
    	see <a href="console-troubleshooting.html#multi-T0-deployments">{{  vars.product }} Management Console Cannot Retrieve Cluster Data in a Multi-Tier0 Topology</a>
...
- If your VMware NSX environment uses custom certificates, obtain the CA certificate for NSX Manager.
    <p class="note"><strong>Note</strong>: If VMware NSX uses custom certificates and you do not provide the CA certificate for NSX Manager,
    	{{  vars.product }} Management Console automatically generates one and registers it with NSX Manager.
...
The following are VMware NSX requirements for deploying the {{  vars.product }} Management Console:
