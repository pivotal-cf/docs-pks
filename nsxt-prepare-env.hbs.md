---
title: Network Planning for Installing +AHsAew-  vars.product +AH0AfQ- with VMware NSX

---

This topic describes how to plan your environment before installing +AHsAew-  vars.product+AF8-full +AH0AfQ- (+AHsAew- vars.product+AF8-short +AH0AfQ-) on VMware vSphere with NSX integration.


+ACMAIwA8-a id+AD0-'overview'+AD4APA-/a+AD4-Overview

Before installing +AHsAew-  vars.product+AF8-full +AH0AfQ- on VMware vSphere with NSX integration, plan your environment as described in the following sections:
...
+ACo- +AFsAewB7- vars.platform+AF8-name +AH0AfQ- documentation+AF0-(https://techdocs.broadcom.com/us/en/vmware-tanzu/platform/tanzu-operations-manager/3-1/tanzu-ops-manager/vsphere-deploy.html)
...
Review the following +AHsAew-  vars.product +AH0AfQ- documentation:

+ACo- +AFs-VMware vSphere with NSX Version Requirements+AF0-(vsphere-nsxt-requirements.html)
+ACo- +AFs-Hardware Requirements for +AHsAew-  vars.product +AH0AfQ- on VMware vSphere with NSX+AF0-(vsphere-nsxt-rpd-mpd.html)
+ACo- +AFs-VMware Ports and Protocols+AF0-(https://ports.vmware.com/home/vSphere+NSU-Data-Center-for-vSphere+NSU-Data-Center)
on the VMware site.
+ACo- +AFs-Network Objects Created by NSX for +AHsAew-  vars.product +AH0AfQBd-(./vsphere-nsxt-cluster-objects.html)


+ACMAIwA8-a id+AD0-'components'+AD4APA-/a+AD4-Understand Component Interactions

+AHsAew-  vars.product +AH0AfQ- on VMware vSphere with NSX requires the following component interactions:
...
+ACo- Each +AHsAew-  vars.product +AH0AfQ--provisioned Kubernetes cluster deploys the NSX Node Agent and the Kube Proxy that run as BOSH-managed processes on each worker node.
...
Review the +AFs-Deployment Topologies+AF0-(nsxt-topologies.html) for +AHsAew-  vars.product +AH0AfQ- on VMware vSphere with NSX. The most common deployment topology is the +AFs-NAT topology+AF0-(./nsxt-topologies.html+ACM-topology-nat). Decide which deployment topology you will implement, and plan accordingly.


+ACMAIwA8-a id+AD0-'plan-cidrs'+AD4APA-/a+AD4-Plan Network CIDRs

Before you install +AHsAew-  vars.product +AH0AfQ- on VMware vSphere with NSX, plan the CIDRs and IP blocks that you are using in your deployment.
...
+ACo- +ACoAKg-TKGI MANAGEMENT CIDR+ACoAKg-: This small network is used to access +AHsAew-  vars.product +AH0AfQ- management components such as +AHsAew- vars.platform+AF8-name +AH0AfQ-, BOSH Director, and +AHsAew-  vars.product +AH0AfQ- VMs as well as the Harbor Registry VM if deployed. For example, a CIDR of +AGA-10.172.1.0/28+AGA- provides 14 usable IPs. For the +AFs-No-NAT deployment topologies+AF0-(nsxt-topologies.html+ACM-topology-no-nat-virtual-switch), this is a corporate routable subnet /28. For the +AFs-NAT deployment topology+AF0-(nsxt-topologies.html+ACM-topology-nat), this is a non-routable subnet /28, and DNAT needs to be configured in NSX to access the +AHsAew-  vars.product +AH0AfQ- management components.

+ACo- +ACoAKg-TKGI LB CIDR+ACoAKg-: This network provides your load balancing address space for each Kubernetes cluster created by +AHsAew-  vars.product +AH0AfQ-. The network also provides IP addresses for Kubernetes API access and Kubernetes exposed services. For example, +AGA-10.172.2.0/24+AGA- provides 256 usable IPs. This network is used when creating the +AGA-ip-pool-vips+AGA- described in +AFs-Creating VMware NSX Objects for +AHsAew-  vars.product +AH0AfQBd-(nsxt-create-objects.html), or when the services are deployed. You enter this network in the
+ACoAKg-Floating IP Pool ID+ACoAKg- field in the +ACoAKg-Networking+ACoAKg- pane of the +AHsAew-  vars.product +AH0AfQ- tile.


+ACMAIwA8-a id+AD0-'plan-ip-blocks'+AD4APA-/a+AD4-Plan IP Blocks

When you install +AHsAew-  vars.product +AH0AfQ- on VMware NSX, you are required to specify the +ACoAKg-Pods IP Block ID+ACoAKg- and +ACoAKg-Nodes IP Block ID+ACoAKg- in the +ACoAKg-Networking+ACoAKg- pane of the +AHsAew-  vars.product +AH0AfQ- tile.
...
For more information, see the +AFs-Networking+AF0-(installing-nsx-t.html+ACM-networking) section of +AF8-Installing +AHsAew-  vars.product +AH0AfQ- on VMware vSphere with NSX Integration+AF8-.
...
After you deploy +AHsAew-  vars.product +AH0AfQ-, you can define a network profile that specifies a routable IP block for your pods.
...
Each Kubernetes cluster deployed by +AHsAew-  vars.product +AH0AfQ- owns a /24 subnet.

To deploy multiple Kubernetes clusters, set the +ACoAKg-Nodes IP Block ID+ACoAKg- in the +ACoAKg-Networking+ACoAKg- pane of the +AHsAew-  vars.product +AH0AfQ- tile to larger than /24. The recommended size is /16. For more information, see +AFs-Creating VMware NSX Objects for +AHsAew-  vars.product +AH0AfQBd-(nsxt-create-objects.html).
...
+ADw-table+AD4-
  ...
  +ADw-tr+AD4-
    ...
    +ADw-td+AD4-No.+ADw-/td+AD4-
    +ADw-td+AD4-
      containerd is installed on each +AHsAew-  vars.product +AH0AfQ- worker node and is assigned the +ADw-code+AD4-172.17.0.0/16+ADw-/code+AD4- network interface.
      +ADw-br+AD4APA-br+AD4-Do not use this CIDR range for any TKGI component, including +AHsAew- vars.platform+AF8-name +AH0AfQ-, BOSH Director, the TKGI API VM, the TKGI DB VM, and the Harbor Registry VM.
  ...
  +ADw-tr+AD4-
    ...
    +ADw-td+AD4-
      The +AHsAew-  vars.product +AH0AfQ- Management Console runs the Docker daemon and reserves +ADw-code+AD4-172.18.0.0/16+ADw-/code+AD4- for the subnet.
  ...
  +ADw-tr+AD4-
    ...
    +ADw-td+AD4-Yes.+ADw-br+AD4-See OVA configuration.+ADw-/td+AD4-
    +ADw-td+AD4-
      The +AHsAew-  vars.product +AH0AfQ- Management Console runs the Docker daemon and reserves +ADw-code+AD4-172.18.0.1+ADw-/code+AD4- for the gateway.
...
To install +AHsAew-  vars.product +AH0AfQ- on vSphere with VMware NSX, you will need to know the following:

+ACo- Subnet name where you will install +AHsAew-  vars.product +AH0AfQ-
