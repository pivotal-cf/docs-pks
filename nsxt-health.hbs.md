...
This topic describes how cluster managers and users can troubleshoot NSX networking errors using the `kubectl nsxerrors` command for {{  vars.product_full }}.

## <a id='nsxerrors-about'></a>About the NSX Errors CRD

The NSX Errors CRD gives you the ability to view errors related to NSX that might occur when applications are deployed to a {{ vars.product_short }}-provisioned Kubernetes cluster. Previously, NSX errors were logged in NCP logs on the control plane nodes, which cluster users do not have access to. The NSX Errors CRD improves visibility and troubleshooting for cluster managers and users.
