...
{{/ evalExpression }}
* **Telegraf**: To configure Telegraf, see [Configuring Telegraf in {{ vars.product_short }}](monitor-etcd.html). The Telegraf agent sends metrics from {{ vars.product_short }} API, control plane node, and worker node VMs to a monitoring service, such as Datadog.
...
1. Under **Enable Syslog for {{ vars.product_short }}**, select **Yes**.
