...
<p class="note"><strong>Note:</strong> If a cluster manager, <code>pks.clusters.manage</code>, attempts to create or delete a network profile,
    ...
    Please contact the TKGI administrator.</i>"
</p>
...
* [Add a New Network Profile to a Cluster that Does Not Have a Network Profile](#new-np-existing-cluster)
...
* `CNI-CONFIGURATIONS-PARAMETERS`  are one or more comma-delimited
    ...
    * If you plan to run [Tanzu Application Platform](https://techdocs.broadcom.com/us/en/vmware-tanzu/standalone-components/tanzu-application-platform/1-12.html) (TAP) on TKGI-provisioned clusters with NSX networking, set the parameter `extensions.ncp.k8s.label_filtering_regex_list` as described under [label_filtering Settings](#label-filtering).
...
TKGI uses strict validation rules before applying a network profile to a cluster
...
## <a id="new-np-existing-cluster"></a> Add a New Network Profile to a Cluster that Does Not Have a Network Profile
...
> **Important** To add pod IPs to a cluster that is already using a network profile, see [Add Pod IPs](./network-profiles-pods.html#pods-ips-add).
...
<table>
    ...
    <tr>
        ...
        <td>Array of floating IP pool UUIDs defined in NSX.</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        If DNS mode is set to <code>API_INGRESS</code>, TKGI creates the cluster with
    ...
    <tr>
        ...
        <td>String</td>
        <td>Size of the NSX load balancer service.<br>Values: <code>"small"</code>, <code>"medium"</code>, <code>"large"</code>.<br>Default: <code>"small"</code>.</td>
    </tr>
    <tr>
        <td><code>master_vms_nsgroup_id</code></td>
        <td>String</td>
        <td>Namespace Group UUID as defined in NSX.</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>String</td>
        <td>Tenant Tier-0 Router UUID defined in NSX.</td>
    </tr>
</table>
...
<table>
    ...
    <tr>
        ...
        <td>String<br><strong>Updatable</strong></td>
        <td>UUID of the bottom <code>section-id</code> for the distributed firewall (DFW) section as defined in NSX.<br>See also: <code>top_firewall_section_marker</code>
            below and <a href="network-profiles-ncp-dfw.html">Define DFW Section Markers</a>.</td>
    </tr>
    <tr>
        <td><code>client_ssl_profile</code></td>
        <td>String<br><strong>Updatable</strong></td>
        <td>The NSX client-side ssl profile to use, exposed by NCP as <code>client_ssl_profile</code>.
    ...
    <tr>
        ...
        <td>Boolean<br><strong>Updatable</strong></td>
        <td>Enable NSX load balancer TCP multiplexing.<br>Values: <code>true</code>, <code>false</code>.<br>Default: <code>false</code>.</td>
    </tr>
    <tr>
        <td><code>lb_connection_multiplexing_number</code></td>
        <td>Integer<br><strong>Updatable</strong></td>
        <td>The maximum number of NSX load balancer TCP multiplexing connections.<br>Default: <code>6</code>.</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td><strong>Deprecated</strong>.
            <br>Use NSX layer 7 virtual server as the ingress controller for the Kubernetes cluster.
    ...
    <tr>
        ...
        <td>String<br><strong>Updatable</strong></td>
        <td>UUID of the top <code>section-id</code> for the distributed firewall (DFW) section as defined in NSX.<br>See also: <code>bottom_firewall_section_marker</code>
...
{{{{raw}}}} <!-- bottom&#95;firewall&#95;section&#95;marker# --> {{{{/raw}}}}
<p class="note"><strong>Note:</strong> On an existing cluster, you can modify
    only the network profile parameters that are labeled <b>Updatable</b>.
</p>
...
For more information about any of these parameters, see [The nsx-ncp-config ConfigMap](https://techdocs.broadcom.com/us/en/vmware-cis/nsx/event-catalog/3-0/nsx-container-plugin-for-kubernetes-and-tanzu-application-service/overview/the-nsx-node-agent-config-configmap.html)
and [The nsx-node-agent-config ConfigMap](https://techdocs.broadcom.com/us/en/vmware-cis/nsx/event-catalog/3-0/nsx-container-plugin-for-kubernetes-and-tanzu-application-service/overview/the-nsx-node-agent-config-configmap.html)
...
{{{{raw}}}} <!--   <tr><td>ncp.coe.adaptor</td><td>String</td><td>Container orchestrator adaptor to plug in.    <br>Values: <code>kubernetes</code>, <code>openshift</code>, <code>pcf</code>.    <br>Default: <code>kubernetes</code>.</td></tr>    # --> {{{{/raw}}}}
{{{{raw}}}} <!--   <tr><td>ncp.coe.node_type</td><td>String</td><td>Container host node type.    <br>Values: <code>BAREMETAL</code>, <code>CLOUD</code>, <code>HOSTVM</code>.    <br>Default: <code>HOSTVM</code>.</td></tr>     # --> {{{{/raw}}}}
{{{{raw}}}} <!--   <tr><td>ncp.coe.profiling</td><td>Boolean<br><strong>Updatable</strong></td>    <td>Values: <code>TRUE</code>, <code>FALSE</code>.    <br>Default: <code>FALSE</code>.</td></tr>  # --> {{{{/raw}}}}
...
<tr><td>ncp.k8s.label_filtering_regex_list</td><td>String List<br><strong>Updatable</strong></td>
    <td>List of regex expressions defining the labels that must not be converted to NSX tags. See [label_filtering Settings](#label-filtering) below for guidance.
...
<tr><td>ncp.nsx_v3.l4_lb_auto_scaling</td><td>Boolean<br><strong>Updatable</strong></td>
    <td>L4 load balancer auto scaling mode.
        ...
        For additional transparent mode requirements, see [Limitations](https://techdocs.broadcom.com/kr/ko/vmware-cis/nsx/event-catalog/4-2/release-notes/vmware-nsx-container-plugin-4211-release-notes.html#Limitations)
...
#### <a id='label-filtering'></a> label_filtering Settings
...
is to configure which NSX client-side ssl profile is to be used by NCP.
...
is to add additional NSX floating IP pool UUIDs, for a cluster or load balancer, to a cluster.
...
is to enable NSX load balancer TCP multiplexing.
...
is to configure the maximum number of NSX load balancer TCP multiplexing connections.
