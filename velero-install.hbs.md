...
This topic describes how to install Velero for backing up and restoring {{  vars.product }}-provisioned Kubernetes workloads.
...
1. Run the following command to install Velero on the target Kubernetes cluster:
    ...
    ```
    velero install --image projects.packages.broadcom.com/tkg/velero/velero:{{{ vars.velero_version }}}_vmware.1
    --provider aws \
...
1. Push the Docker images into the internal registry. Adjust the variables as needed for your registry instance and preferences.
    ...
    docker load -i velero-restore-helper-{{{ vars.velero_version }}}+vmware.1.tar.gz
    docker tag projects.packages.broadcom.com/tkg/velero/velero-restore-helper:{{{ vars.velero_version }}}_vmware.1 harbor.example.com/vmware-tanzu/velero-restore-helper:{{{ vars.velero_version }}}_vmware.1
    ...
    docker tag projects.packages.broadcom.com/tkg/velero/velero:{{{ vars.velero_version }}}_vmware.1 harbor.example.com/vmware-tanzu/velero:{{{ vars.velero_version }}}_vmware.1
    docker push harbor.example.com/vmware-tanzu/velero-plugin-for-aws:{{{ vars.velero_version_aws }}}_vmware.1
    docker push harbor.example.com/vmware-tanzu/velero-restore-helper:{{{ vars.velero_version }}}_vmware.1
