In the procedure below, you use credentials for vCenter master VMs. You must have provisioned the service account with the correct permissions. For more information, see [Create the Master Node Service Account](vsphere-prepare-env.html#create-master) in _Preparing vSphere Before Deploying {{  vars.product }}_.
...
1. Ensure the values in the following procedure match those in the **vCenter Config** section of the {{ vars.platform_name }} tile:
    ...
    For more information about the master node service account, see [Preparing vSphere Before Deploying {{  vars.product }}](vsphere-prepare-env.html).
