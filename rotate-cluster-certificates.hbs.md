...
{{  vars.product_full }} Kubernetes clusters.

For more information about {{ vars.product_short }} Certificates:
...
* To rotate the certificates used by the {{ vars.product_short }} control plane,
see [Rotating {{ vars.product_short }} Control Plane Certificates](./rotate-tile-certificates.html).

## <a id="overview"></a>Overview

When {{ vars.product_short }} provisions a Kubernetes cluster, the system generates
certificate authority (CA) certificates and leaf certificates that have values and
expiration dates unique to that cluster.

You can use the {{ vars.product_short }} CLI to manage and rotate
{{ vars.product_short }}-provisioned Kubernetes cluster certificates.
...
<table id='cluster-certs' border="1" class="nice" >
  <tr>
    ...
    <td><code>kubo_master_ca_2021</code>, <code>kubo_ca_2018</code>, <code>front_proxy_2024</code>, <code>etcd_ca_2018</code>, and their leaf certificates</td>
    <td>All clusters.</td>
    ...
    <td>NSX only. These certificates must be registered with NSX Manager.</td>
    <td>See <a href="nsxt-certs-rotate.html">Rotate NSX Certificates for Kubernetes Clusters</a>.</td>
  </tr>
</table>

For more information about Kubernetes Cluster certificates in {{ vars.product_short }},
see [{{ vars.product_short }} Certificates](certificate-concepts.html).
...
To rotate {{ vars.product_short }}-provisioned Kubernetes cluster certificates,
...
To list the TLS certificates used by {{ vars.product_short }}-provisioned Kubernetes cluster, run the following command:
...
The sample output lists all TLS certificates that {{ vars.product_short }} uses for the specified cluster.
...
The {{ vars.product_short }} CLI supports rotating TLS certificates for the following scenarios:
...
<br>
For more information about how to use {{ vars.product_short }} CLI to rotate Kubernetes cluster TLS certificates,
see [Rotate TLS Certificates Using the {{ vars.product_short }} CLI](#certs-rotate) below.
...
### <a id="rotate-only-nsx"></a>Rotate NSX Certificates Only
...
This command only rotates the NSX certificates `tls-nsx-t` and `tls-nsx-lb`.
...
###<a id='certs-rotate'></a> Rotate TLS Certificates Using the {{ vars.product_short }} CLI

You can use the {{ vars.product_short }} CLI to list and rotate the TLS certificates created for a Kubernetes cluster.
