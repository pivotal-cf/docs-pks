...
* For conceptual information about certificates in TKGI, see [TKGI Certificates](certificate-concepts.html).
  * To keep TKGI running, you need to monitor expiration dates and rotate all of the certificate types listed on the [TKGI Certificates](certificate-concepts.html) page, including BOSH certificates that are not created by TKGI.
...
so users do not ordinarily need to rotate them.
    * To rotate **Non-configurable** certificates, use the Rotate Control Plane Certificates Tool.
    For more information, see [How to rotate TKGI control plane CA and leaf certificates](https://knowledge.broadcom.com/external/article?legacyId=88309)
