---
title: Back Up and Restore Stateful App with Static IP for Load Balancer Service (CSI Edition)
owner: TKGI
---
...
Edit the **wordpress-deployment.yaml** to include the static IP for the load balancer (`loadBalancerIP: IP`). In this example, the IP address used is taken from the NSX floating IP pool that is resourced for TKGI.
