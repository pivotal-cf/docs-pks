...
provisioned with {{  vars.product_full }}, or
...
**In-Cluster Monitoring**:
  ...
  * If you want to create a `ClusterMetricSink` or `MetricSink` resource,
  select the **Enable Metric Sink Resources** check box.
  * If you want to use Node Exporter to send worker node metrics to
  ...
  below, select the **Enable node exporter on workers** check box.
  <br><br>
...
<p class="note"><strong>Note:</strong> Log sinks created in TKGI do not support UDP connections.</p>

<p class="note"><strong>Note:</strong> TKGI requires a secure connection
...
have to use a namespaced MetricSink. A MetricSink creates a unique TSelegraf
