...
On vSphere, you can add individual users or user groups to {{  vars.product }} Management Console. You can assign roles to individual users or to groups. If you assign a role to a group, all of the users in that group have that role.

For information about the roles that you can assign, see [UAA Scopes for {{  vars.product }} Users](./uaa-scopes.html).

For information about the tasks that Cluster Managers can perform, see [{{  vars.product }} Architecture](./control-plane.html). The TKGI Administrator role allows users to manage the {{  vars.product }} infrastructure.

## <a id='identity-users'></a>Add Individual Users

The procedure to add individual users to {{  vars.product }} Management Console is as follows.

<p class="note"><strong>Note</strong>: This release of {{  vars.product }} Management Console does not support assigning roles to individual LDAP or SAML users. To assign roles to LDAP or SAML users, use user groups.</p>
...
The procedure to add user groups to {{  vars.product }} Management Console is as follows.
...
The procedure to remove individual users from {{  vars.product }} Management Console is as follows.
