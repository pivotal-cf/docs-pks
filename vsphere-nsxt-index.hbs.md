...
In preparation for installing {{  vars.product }} on vSphere with VMware NSX, review all of the topics in the subsection [Preparing to Install {{  vars.product }} on vSphere with VMware NSX](./vsphere-nsxt-index-prepare.html).

##<a id='step02'></a> Step 2: Install and Configure VMware NSX for {{  vars.product }}
...
Create vSphere Resource Pools for the Availability Zones where you will deploy Kubernetes clusters. These resource pools map to the AZs you will create when you configure BOSH Director and reference when you install the {{  vars.product }} tile.
...
For instructions, see <a href="./vsphere-nsxt-om-deploy.html">Deploying {{ vars.platform_name }} with NSX for {{  vars.product }}</a>.
...
For instructions, see <a href="./vsphere-nsxt-om-config.html">Configuring BOSH Director with NSX for {{  vars.product }}</a>.
...
##<a id='step09'></a> Step 9: Install {{  vars.product }} on vSphere with NSX

At this point your NSX environment is prepared for {{  vars.product }} installation using the {{  vars.product }} tile in {{ vars.platform_name }}.

For instructions, see <a href="./installing-nsx-t.html">Installing {{  vars.product }} on vSphere with NSX</a>.
...
See <a href="vsphere-configure-users.html">Setting Up {{  vars.product }} Admin Users on vSphere</a>
...
After you have installed {{  vars.product }} on vSphere with NSX, refer to the following subsection for topics describing additional NSX configuration options: [Advanced Configurations for {{  vars.product }} on vSphere with VMware NSX](./vsphere-nsxt-index-adv.html)
