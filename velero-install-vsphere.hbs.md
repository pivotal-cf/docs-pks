...
The Velero back up procedure requires an object store as the back up destination for workload backups. Deploy and configure a MinIO Server on a Linux Ubuntu VM as the Velero backend object store.
...
1. Install Velero on the target Kubernetes cluster:
    ...
    ```
    velero install \
    --image projects.packages.broadcom.com/tkg/velero/velero:{{{ vars.velero_version }}}_vmware.1
    --provider aws --bucket tkgi-velero \
    --secret-file ./credentials-minio \
    --plugins "projects.packages.broadcom.com/tkg/velero/velero-plugin-for-aws:{{{ vars.velero_version_aws }}}_vmware.1" \
...
1. Download the Velero CLI and Velero Plugin for vSphere images for your version of TKGI:
    ...
    data-manager-for-plugin-{{{ vars.velero_version_data_manager }}}_vmware.1.tar.gz
    velero-plugin-for-vsphere-{{{ vars.velero_version_vsphere }}}_vmware.1.tar.gz
    ```
...
1. Push the Docker images into the internal registry. Adjust the variables as needed for your registry instance and preferences.
    ...
    docker load -i backup-driver-{{{ vars.velero_version_backup }}}_vmware.1.tar.gz
    docker tag vmware.io/backup-driver:{{{ vars.velero_version_backup }}}_vmware.1  harbor.example.com/vmware-tanzu/backup-driver:{{{ vars.velero_version_backup }}}_vmware.1
    docker load -i velero-plugin-for-vsphere-{{{ vars.velero_version_vsphere }}}_vmware.1.tar.gz
    docker tag vmware.io/velero-plugin-for-vsphere:{{{ vars.velero_version_vsphere }}}_vmware.1  harbor.example.com/vmware-tanzu/velero-plugin-for-vsphere:{{{ vars.velero_version_vsphere }}}_vmware.1
    docker load -i data-manager-for-plugin-{{{ vars.velero_version_data_manager }}}_vmware.1.tar.gz
    docker tag vmware.io/data-manager-for-plugin:{{{ vars.velero_version_data_manager }}}_vmware.1  harbor.example.com/vmware-tanzu/data-manager-for-plugin:{{{ vars.velero_version_data_manager }}}_vmware.1
    docker push harbor.example.com/vmware-tanzu/backup-driver:{{{ vars.velero_version_backup }}}_vmware.1
    docker push harbor.example.com/vmware-tanzu/velero-plugin-for-vsphere:{{{ vars.velero_version_vsphere }}}_vmware.1
