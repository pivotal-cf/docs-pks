...
* The following network profile uses the NSX Load Balancer and NSX Ingress Controller:

    ```
    {
      "name": "example_network_profile",
      "description": "Use the NSX Load Balancer and NSX Ingress Controller",
      "parameters": {
        "cni_configurations": {
          "type": "nsxt",
          "parameters": {
...
If `persistence_type` and `persistence_timeout` are not both specified, the network profile fails validation.
  ...
  <table>
    ...
    <tr>
      ...
      <td>String</td>
      <td>Valid values are <code>"none"</code>, <code>"cookie"</code> or <code>"source_ip"</code>. An empty value is not accepted.</td>
    </tr>
    <tr>
